...
<h1 style="text-align: center;">Living Room Furniture</h1>
<div class="row">
<%
try {
    String id, name, brand, price, color, cat, dimensions, image;
    PreparedStatement pst = cn.prepareStatement("SELECT * FROM product WHERE category = ?");
    pst.setString(1, "Living Room"); // Filter to show only Living Room furniture
...
%>
    <div class="col-sm-3">
        <div class="product-box">
            ...
            <center><a class="btn btn-primary" href="buy.jsp?id=<%=id %>&price=<%=price %>&name=<%=name %>&category=<%=cat %>&brand=<%=brand %>&color=<%=color %>&dimensions=<%=dimensions %>>">Buy Now</a></center>
        </div>
    </div>
<%
    }
